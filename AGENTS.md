...
**Nexus AI Chat Importer** is an Obsidian plugin that imports AI chat conversations (ChatGPT and Codex) as beautifully formatted Markdown files with full attachment support, metadata preservation, and intelligent deduplication.
...
2. **ProviderAdapter** (ChatGPT/Codex) - Detects and converts provider-specific format
...
1. **ProviderSelectionDialog** - Choose ChatGPT or Codex
...
**Codex Provider** ([src/providers/Codex/](src/providers/Codex/)):
- `Codex-adapter.ts` - Main adapter implementation
- `Codex-converter.ts` - Converts Codex JSON to StandardConversation
- `Codex-attachment-extractor.ts` - Extracts attachments and artifacts
- `Codex-types.ts` - Codex-specific TypeScript types
...
- Formats provider-specific features (DALL-E prompts, Codex artifacts)
...
This updates all references across the vault, including links within conversation notes and Codex artifact files.
