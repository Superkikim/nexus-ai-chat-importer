...
Every import writes a report so you can see exactly what happened. Reports go to
`<Report folder>/<provider>/` (default `Nexus/Reports/chatgpt/`, etc.).

## The three files

Each import creates three Markdown files sharing a timestamp prefix
...
you get all three on desktop and mobile.

All three carry frontmatter with the import date, provider, mode (all / selective),
archive counts, note outcomes, and cross-links to the other two.
...
counts, and a **View Report** button that opens the summary.

If Nexus cannot create the report folder or write the reports, it shows a failure
notice and a short "import completed" notice instead — there is no completion
dialog with a report link in that case.
