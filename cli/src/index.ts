...
function parseImportArgs(args: string[]): ImportOptions {
    ...
    while (i < args.length) {
        ...
        switch (arg) {
            ...
            case "--conversation-folder": {
                options.conversationFolder = requireValue(
                    args,
                    i,
                    "--conversation-folder"
                );
            ...
            case "--attachment-folder": {
                options.attachmentFolder = requireValue(
                    args,
                    i,
                    "--attachment-folder"
                );
            ...
            case "--date-format": {
                options.dateFormat = requireValue(
                    ...
                    i,
                    "--date-format"
                ) as ImportOptions["dateFormat"];
                i += 2;
                break;
            }
            case "--timestamp-format": {
                options.timestampFormat = requireValue(
                    args,
                    i,
                    "--timestamp-format"
                );
    ...
    if (
        !VALID_PROVIDERS.includes(
            options.provider as (typeof VALID_PROVIDERS)[number]
        )
    ) {
        throw new Error(
            `Invalid provider "${
    ...
    if (
        ...
        throw new Error(
            `Invalid timestamp format "${
                options.timestampFormat
            }". Must be one of: ${VALID_TIMESTAMP_FORMATS.join(", ")}`
        );
    }
