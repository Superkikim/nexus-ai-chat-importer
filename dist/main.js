  ...
  async processConversationStream(provider, generator, importReport, zip, isReprocess = false, selectedIds, progressCallback, approxTotal) {
    ...
    for await (const chat of generator) {
      ...
      progressCallback == null ? void 0 : progressCallback({
        ...
        detail: approxTotal ? `Processing conversation ${processedCount} of ${approxTotal}` : `Processing conversation ${processedCount}`,
        current: processedCount,
        total: approxTotal
      });
    }
...
// src/utils/zip/desktop-zip-reader.ts
...
var DesktopZipEntryHandle = class {
  constructor(filePath, entryName) {
...
var DesktopZipArchiveReader = class {
...
__name(DesktopZipArchiveReader, "DesktopZipArchiveReader");
async function readDesktopZipEntries(filePath, shouldInclude) {
  const zipfile = await openYauzl(filePath);
  return await new Promise((resolve, reject) => {
    const entries = [];
...
__name(readDesktopZipEntries, "readDesktopZipEntries");

// src/utils/zip/mobile-zip-reader.ts
init_logger();
...
__name(readZip64CentralDirectoryInfo, "readZip64CentralDirectoryInfo");
...
__name(readCentralDirectoryInfo, "readCentralDirectoryInfo");
function parseZip64ExtraField(extraFieldBytes, needs) {
...
__name(parseZip64ExtraField, "parseZip64ExtraField");
async function parseCentralDirectory(file, info) {
  var _a, _b, _c;
  ...
  while (pos + 46 <= buffer.byteLength && entries.length < info.entryCount) {
    ...
    if (compressedSize === 4294967295 || uncompressedSize === 4294967295 || localHeaderOffset === 4294967295) {
      const zip64Values = parseZip64ExtraField(extraFieldBytes, {
        uncompressedSize: uncompressedSize === 4294967295,
        compressedSize: compressedSize === 4294967295,
        localHeaderOffset: localHeaderOffset === 4294967295
      });
      compressedSize = (_a = zip64Values.compressedSize) != null ? _a : compressedSize;
      uncompressedSize = (_b = zip64Values.uncompressedSize) != null ? _b : uncompressedSize;
      localHeaderOffset = (_c = zip64Values.localHeaderOffset) != null ? _c : localHeaderOffset;
    }
...
__name(inflateRawDeflate, "inflateRawDeflate");
async function readLocalFileData(file, entry) {
...
var MobileZipArchiveReader = class {
...
__name(readMobileZipEntries, "readMobileZipEntries");

// src/utils/zip/attachment-lookup-index.ts
...
__name(extractFileIds, "extractFileIds");
function buildAttachmentLookupIndex(entries) {
  const index = {
    byExactPath: /* @__PURE__ */ new Map(),
    byBaseName: /* @__PURE__ */ new Map(),
    byFileId: /* @__PURE__ */ new Map(),
    byDalleId: /* @__PURE__ */ new Map()
  };
  for (const entry of entries) {
...
__name(buildAttachmentLookupIndex, "buildAttachmentLookupIndex");

// src/utils/zip/write-zip-entry-to-vault.ts
async function writeZipEntryToVault(entry, targetPath, vault) {
  var _a, _b;
  let bytes = await entry.readBytes();
  const detected = detectFileFormat(bytes);
  const result = {
    byteLength: bytes.byteLength,
    detectedMimeType: (_a = detected.mimeType) != null ? _a : void 0,
    detectedExtension: (_b = detected.extension) != null ? _b : void 0
  };
  const resolvedTargetPath = typeof targetPath === "function" ? await targetPath(result) : targetPath;
  await vault.adapter.writeBinary(resolvedTargetPath, bytes.buffer);
  bytes = null;
  return {
    ...result,
...
__name(writeZipEntryToVault, "writeZipEntryToVault");

// src/utils/zip/index.ts
async function createZipArchiveReader(file, shouldInclude) {
...
__name(createZipArchiveReader, "createZipArchiveReader");
async function enumerateZipEntries(file, shouldInclude) {
...
var ChatGPTAttachmentExtractor = class {
  constructor(plugin, logger6) {
    ...
    this.allZips = [];
    this.lookupIndexCache = /* @__PURE__ */ new WeakMap();
  }
  /**
...
>> \u26A0\uFE0F ${errorMessage}`
   ...
   */
  async extractSingleAttachment(conversationId, attachment, locatedFile) {
  ...
  findCandidatePaths(index, fileId) {
...
>> \u26A0\uFE0F Not included in archive. [Open original conversation](${conversationUrl})`;
   ...
   */
  async findFileInZip(zip, fileName) {
    if (zip.has(fileName))
      return fileName;
    const commonPaths = ["attachments/", "files/", "uploads/"];
    for (const path of commonPaths) {
      if (zip.has(path + fileName))
...
\`\`\``
   ...
   */
  async saveAttachmentToVault(fileName, zipFile, category = "files") {
    ...
    const writeResult = await writeZipEntryToVault(zipFile, filePath, this.plugin.app.vault);
...
var AttachmentMapBuilder = class {
  constructor(logger6) {
    this.logger = logger6;
    this.attachmentLogger = typeof logger6.child === "function" ? logger6.child("AttachMap") : new Logger().child("AttachMap");
  }
  /**
...
function classifyArchiveEntries(fileNames, forcedProvider) {
  ...
  const hasConversationsJson = fileNames.includes("conversations.json") || fileNames.some((name) => /^conversations-\d+\.json$/.test(name));
  ...
  const detectedProvider = hasLeChatFiles && !hasConversationsJson ? "lechat" : hasGeminiActivityJson && !hasConversationsJson && !hasLeChatFiles ? "gemini" : hasConversationsJson && hasUsersJson ? "claude" : hasConversationsJson ? "chatgpt" : void 0;
  if (forcedProvider) {
...
__name(classifyArchiveEntries, "classifyArchiveEntries");
async function listFileNames(zip) {
  const entries = await zip.listEntries();
...
// src/utils/file-sort.ts
function sortFilesForImport(files) {
...
__name(sortFilesForImport, "sortFilesForImport");
function extractEmbeddedTimestamp(fileName) {
  var _a;
...
Do NOT extract and re-compress the file - just rename it!`;
  ...
  async validateZipFile(file, forcedProvider) {
    ...
    try {
      ...
      importLogger.info(`ZIP classification complete`, {
        ...
        provider: classification.supported ? classification.provider : void 0
      });
  ...
  async processConversations(zip, file, isReprocess, forcedProvider, progressCallback, selectedConversationIds, progressModal, zipSizeBytes) {
    ...
    try {
      ...
      importLogger.info(`Conversation processing strategy selected`, {
        ...
        useStreaming,
        selectedConversationCount: (_a = selectedConversationIds == null ? void 0 : selectedConversationIds.length) != null ? _a : null,
        zipSizeBytes: zipSizeBytes != null ? zipSizeBytes : null
      });
      if (useStreaming) {
        const selectedIds = selectedConversationIds ? new Set(selectedConversationIds) : void 0;
        progressCallback == null ? void 0 : progressCallback({
          phase: "processing",
          title: "Processing conversations...",
          detail: "Streaming conversations from ZIP",
          current: 0,
          total: selectedConversationIds == null ? void 0 : selectedConversationIds.length
        });
        ...
        importLogger.info(`Raw conversations extracted`, {
          ...
          archiveReason: (_c = archiveModeDecision == null ? void 0 : archiveModeDecision.reason) != null ? _c : null
        });
        if (selectedConversationIds && selectedConversationIds.length > 0) {
        ...
        progressCallback == null ? void 0 : progressCallback({
          phase: "processing",
          title: "Processing conversations...",
          detail: (archiveModeDecision == null ? void 0 : archiveModeDecision.mode) === "large-archive" ? "Processing a large archive using a streaming-safe pipeline" : "Converting and importing conversations",
          current: 0,
          total: rawConversations.length
        });
  ...
  async extractRawConversationsFromZip(zip, zipSizeBytes) {
    ...
    const archiveModeDecision = typeof zipSizeBytes === "number" ? decideArchiveMode({
      zipSizeBytes,
...
var ConversationMetadataExtractor = class {
  constructor(providerRegistry, plugin) {
    ...
    this.metadataLogger = typeof pluginLogger.child === "function" ? pluginLogger.child("Metadata") : new Logger().child("Metadata");
  }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const metadata = await this.extractMetadataFromZip(
          zip,
          (_c = classification.provider) != null ? _c : forcedProvider,
          file.name,
          i,
          void 0
        );
...
`;
   ...
   */
  groupConversationsByFile(selectedIds, conversations) {
    const conversationsByFile = /* @__PURE__ */ new Map();
    const selectedIdsSet = new Set(selectedIds);
