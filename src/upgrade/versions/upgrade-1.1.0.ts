...
class DeleteCatalogOperation extends UpgradeOperation {
    ...
    async execute(context: UpgradeContext): Promise<OperationResult> {
        try {
            ...
            if (
                Object.keys(verifyArchives).length === 0 &&
                Object.keys(existingImportedArchives || {}).length > 0
            ) {
                logger.error(
                    `DeleteCatalog: CRITICAL - importedArchives were lost during save!`
                );
                return {
                    success: false,
                    message: `Critical error: importedArchives were lost during migration`,
                    details: {
                        beforeCount: Object.keys(existingImportedArchives || {})
                            .length,
            ...
            return {
                ...
                message: `Legacy catalog deleted: ${catalogSize} entries removed, ${
                    Object.keys(verifyArchives).length
                } imported archives preserved`,
                details: {
                    entriesDeleted: catalogSize,
                    archivesPreserved: Object.keys(verifyArchives).length,
                },
            };
        } catch (error) {
...
class CleanMetadataOperation extends UpgradeOperation {
    ...
    readonly description =
        "Remove unnecessary metadata and add plugin version to conversation notes";
    readonly type = "automatic" as const;
            ...
            for (let i = 0; i < conversationFiles.length; i += batchSize) {
                ...
                for (const file of batch) {
                    ...
                    try {
                        ...
                        const cleanedContent = this.cleanFrontmatterRobust(
                            content,
                            context.toVersion,
                            file.basename
                        );
                        ...
                        errors++;
                        logger.error(
                            `Error cleaning metadata for ${file.path}:`,
                            error
                        );
                    }
                }
     ...
     */
    private cleanFrontmatterRobust(
        content: string,
        ...
        if (!frontmatterMatch) {
            ...
            if (
                ...
                content.includes("conversation_id:")
            ) {
                logger.warn(
                    `File ${fileName} appears to be Nexus but has malformed frontmatter`
                );
            }
            return content;
        }
        ...
        if (frontmatterData.conversation_id) {
            newFrontmatter.push(
                `conversation_id: ${frontmatterData.conversation_id}`
            );
        }
        ...
        try {
            ...
            for (const file of conversationFiles) {
                try {
                    ...
                    logger.error(
                        `CleanMetadata.verify error for ${file.path}:`,
                        error
                    );
                    return false;
                }
