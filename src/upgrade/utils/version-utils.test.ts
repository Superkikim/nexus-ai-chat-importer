...
describe("VersionUtils.compareVersions", () => {
    ...
    it("returns positive when v1 > v2", () => {
        expect(VersionUtils.compareVersions("1.3.1", "1.3.0")).toBeGreaterThan(
            0
        );
        expect(VersionUtils.compareVersions("1.4.0", "1.3.0")).toBeGreaterThan(
            0
        );
        expect(VersionUtils.compareVersions("2.0.0", "1.3.0")).toBeGreaterThan(
            0
        );
    });
});
...
describe("Version guard boundary (MIN_SUPPORTED_VERSION = 1.3.0)", () => {
    it("1.1.0 is below minimum — guard must fire", () => {
        expect(
            VersionUtils.compareVersions("1.1.0", MIN_SUPPORTED)
        ).toBeLessThan(0);
    });

    it("1.2.9 is below minimum — guard must fire", () => {
        expect(
            VersionUtils.compareVersions("1.2.9", MIN_SUPPORTED)
        ).toBeLessThan(0);
    });

    it("1.0.5 is below minimum — guard must fire", () => {
        expect(
            VersionUtils.compareVersions("1.0.5", MIN_SUPPORTED)
        ).toBeLessThan(0);
    });
    ...
    it("1.4.0 is above minimum — guard must NOT fire", () => {
        expect(
            VersionUtils.compareVersions("1.4.0", MIN_SUPPORTED)
        ).toBeGreaterThan(0);
    });

    it("1.6.4 is above minimum — guard must NOT fire", () => {
        expect(
            VersionUtils.compareVersions("1.6.4", MIN_SUPPORTED)
        ).toBeGreaterThan(0);
    });
});
...
describe("VersionUtils.getVersionsInRange", () => {
    ...
    it("returns only versions > from and <= to", () => {
        const chain = VersionUtils.getVersionsInRange(
            "1.3.0",
            "1.6.4",
            available
        );
