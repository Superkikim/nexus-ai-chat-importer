...
export class VersionUtils {
    ...
    static shouldRunMigration(
        ...
        const migrationTo = migrationToVersion
            ? this.parseVersion(migrationToVersion)
            : userTo;

        // User was in the range that needs this migration
        const wasInRange =
            this.compareVersions(fromVersion, migrationFromVersion) >= 0;

        // User is upgrading to a version that has this migration
        const isUpgradingTo =
            this.compareVersions(toVersion, migrationTo.original) >= 0;
