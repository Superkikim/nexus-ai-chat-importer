...
export class FolderSettingsSection extends BaseSettingsSection {
    ...
    private async handleFolderChange(
        ...
        if (
            ...
            this.showErrorDialog(
                ...
                t("folder_migration.error_target_not_empty.message", {
                    path: newPath,
                })
            );
            // Restore old value in the text field
        ...
        const useEnhancedDialog =
            settingKey === "attachmentFolder" ||
            settingKey === "conversationFolder";

        if (useEnhancedDialog) {
            // Lazy import to avoid loading issues during plugin initialization
            import("../../dialogs/enhanced-folder-migration-dialog")
                .then(({ EnhancedFolderMigrationDialog }) => {
                    const dialog = new EnhancedFolderMigrationDialog(
                        ...
                        async (action: "move" | "keep" | "cancel") => {
                            await this.handleMigrationAction(
                                action,
                                oldPath,
                                newPath,
                                oldFolder,
                ...
                .catch((error) => {
                    this.plugin.logger.error(
                        "Failed to load enhanced dialog:",
                        error
                    );
        ...
        if (action === "move") {
            ...
            try {
                ...
                if (
                    ...
                    await this.updateLinksAfterMove(
                        settingKey,
                        oldPath,
                        newPath
                    );
    ...
    private async updateLinksAfterMove(
        ...
        try {
            ...
            this.plugin.logger.error(
                `[FolderSettings] Failed to update links:`,
                error
            );
            new Notice(
                `⚠️ files moved but some links may not have been updated`
            );
        }
    }
