...
export class NoteFormatter {
    ...
    generateMarkdownContent(conversation: StandardConversation): string {
        ...
        const createTimeStr = new Date(
            conversation.createTime * 1000
        ).toISOString();
        const updateTimeStr = new Date(
            conversation.updateTime * 1000
        ).toISOString();

        // Generate user-friendly timestamps for note body
        const createTimeDisplay = `${formatTimestamp(
            conversation.createTime,
            "date"
        )} at ${formatTimestamp(conversation.createTime, "time")}`;
        const updateTimeDisplay = `${formatTimestamp(
            conversation.updateTime,
            "date"
        )} at ${formatTimestamp(conversation.updateTime, "time")}`;
    ...
    private generateHeader(
        ...
        if (!chatUrl && URL_GENERATORS[conversation.provider]) {
            chatUrl =
                URL_GENERATORS[conversation.provider].generateChatUrl(
                    conversationId
                );
        }
...
`;
    ...
    private generateMessagesContent(
        conversation: StandardConversation
    ): string {
    ...
    private generateRelatedQueriesSection(
        conversation: StandardConversation
    ): string {
