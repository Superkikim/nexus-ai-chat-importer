...
export class MistralVibeAttachmentExtractor {
    ...
    private async processAttachment(
        ...
        if (!zipPathMatch && this.attachmentMap) {
            ...
            if (locations && locations.length > 0) {
                ...
                for (const location of locations) {
                    ...
                    if (
                        zipIndex !== -1 &&
                        location.zipIndex < this.allZips.length
                    ) {
                        const targetZip = this.allZips[location.zipIndex];
                        zipPathMatch = await this.findFileInZip(
                            targetZip,
                            location.path
                        );
        ...
        if (!entry) {
            throw new Error(
                `Attachment entry disappeared from ZIP reader: ${zipPathMatch}`
            );
        }
        ...
        if (
            (!finalFileType || finalFileType === "application/octet-stream") &&
            writeResult.detectedMimeType
        ) {
    ...
    private async findFileInZip(
        ...
        if (!filePath) {
            ...
            const matchingPath = entries
                .map((entry) => entry.path)
                .find((p) => p.toLowerCase() === lowerPath);
            if (matchingPath) {
