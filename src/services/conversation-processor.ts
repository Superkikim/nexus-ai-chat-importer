...
export class ConversationProcessor {
    ...
    constructor(
        ...
        this.noteFormatter = new NoteFormatter(
            plugin.logger,
            plugin.manifest.id,
    ...
    async processRawConversations(
        ...
        const provider =
            forcedProvider ||
            this.providerRegistry.detectProvider(rawConversations);
    ...
    private async processConversationsWithProvider(
        ...
        const conversationsToProcess = rawConversations;
        ...
        try {
            ...
            if (!chatId || chatId.trim() === "") {
                this.plugin.logger.warn(
                    `Skipping conversation with missing ID: ${chatTitle}`
                );
            ...
            const forceReprocess =
                isReprocess || !!reprocessConversationIds?.has(chatId);
            ...
            if (existingEntry) {
                ...
                const filePath = await this.generateFilePathForChat(
                    adapter,
                    chat,
                    isStandardConversation
                );
                resolvedPath = await this.handleNewChat(
                    adapter,
                    chat,
                    filePath,
                    importReport,
            ...
            const previousUpdateTime =
                previousEntry?.update_time ?? previousEntry?.updateTime ?? 0;
            const nextUpdateTime = Math.max(
                previousUpdateTime,
                Number(chatUpdateTime) || 0
            );
            const nextCreateTime =
                previousEntry?.create_time ?? (Number(chatCreateTime) || 0);
            ...
            importReport.addError(
                `Error processing chat: ${chatTitle}`,
                errorMessage
            );
        }
    }
    ...
    private async handleExistingChat(
        ...
        const totalMessageCount = await this.countMessages(
            adapter,
            chat,
            isStandardConversation
        );
    ...
    private updateRelatedQueriesSection(
        ...
        const metadata = conversation.metadata || {};
        const relatedQueries = Array.isArray(
            conversation.metadata?.related_queries
        )
    ...
    private async generateFilePathForChat(
        ...
        const maxBaseNameBytes = Math.max(
            1,
            CONVERSATION_NOTE_FILENAME_MAX_BYTES - markdownExtensionBytes
        );
    ...
    private buildFallbackConversationPath(
        originalPath: string,
        conversationId: string
    ): string {
        const slashIndex = originalPath.lastIndexOf("/");
        const folderPath =
            slashIndex >= 0 ? originalPath.substring(0, slashIndex) : "";
