...
export class ImportService {
    ...
    async handleZipFile(file: File, forcedProvider?: string, selectedConversationIds?: string[], sharedReport?: ImportReport) {
        ...
        try {
            ...
            // eslint-disable-next-line no-console
            console.log(`[NexusAI][${new Date().toISOString().slice(11,23)}] [1/4 catalog] Done — ${fileNames.length} entries listed, 0 bytes decompressed, provider=${forcedProvider ?? 'auto-detect'}`);
            ...
            // On mobile: use LazyZip — only the Central Directory is read upfront
            ...
            if (isMobile) {
                ...
                if (lazyEntries.length === 0) {
                    // ZIP64 or EOCD not found — fall back to full selective load
                    // eslint-disable-next-line no-console
